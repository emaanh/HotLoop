# Task: Ragged softmax-pooling (tiny rare long outliers)
...
Packed ragged data: `v` is `(1185736, 16)` float16 values, `s` is `(1185736,)` float16 scores, and
`offsets` is `(262145,)` int64 segment boundaries (262144 segments, none longer than 512).
For every segment the program softmaxes that segment's scores and returns the score-weighted sum
of its values, shape `(262144, 16)`.

Segment lengths follow `{'kind': 'bimodal', 'p_long': 0.001, 'long': 512, 'short': 4}` (see `workload.py::lengths`). The total number of elements is
always exactly 1185736 and the tensor shapes never change; the pattern of lengths and all values
change with every call. The padded tensor the reference builds has 113x as many
