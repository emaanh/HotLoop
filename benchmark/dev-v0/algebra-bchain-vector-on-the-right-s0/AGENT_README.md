# Task: Matmul chain (bchain vector on the right)
...
A batched product `x @ w1 @ w2 @ y`, float32: `x` is `(64, 512, 512)` and `y` is `(64, 512, 1)` (different for every sample); `w1` `(512, 512)` and `w2` `(512, 512)` are shared across the batch. Values are random and change with every call; shapes are fixed.
