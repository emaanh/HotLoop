# Task: Ragged softmax-pooling (small outliers)
...
Packed ragged data: `v` is `(1187680, 32)` float16 values, `s` is `(1187680,)` float16 scores, and
`offsets` is `(65537,)` int64 segment boundaries (65536 segments, none longer than 1024).
For every segment the program softmaxes that segment's scores and returns the score-weighted sum
of its values, shape `(65536, 32)`.

Segment lengths follow `{'kind': 'bimodal', 'p_long': 0.002, 'long': 1024, 'short': 16}` (see `workload.py::lengths`). The total number of elements is
always exactly 1187680 and the tensor shapes never change; the pattern of lengths and all values
change with every call. The padded tensor the reference builds has 57x as many
