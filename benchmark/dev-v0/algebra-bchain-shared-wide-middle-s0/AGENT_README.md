# Task: Matmul chain (bchain shared wide middle)
...
A batched product `x @ w1 @ w2 @ y`, float32: `x` is `(64, 256, 64)` and `y` is `(64, 64, 256)` (different for every sample); `w1` `(64, 2048)` and `w2` `(2048, 64)` are shared across the batch. Values are random and change with every call; shapes are fixed.
