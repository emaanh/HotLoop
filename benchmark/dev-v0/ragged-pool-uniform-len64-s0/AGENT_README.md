# Task: Ragged softmax-pooling (uniform len64)
...
`offsets` is `(16385,)` int64 segment boundaries (16384 segments, none longer than 64).
...
Segment lengths follow `{'kind': 'uniform', 'len': 64}` (see `workload.py::lengths`). The total number of elements is
