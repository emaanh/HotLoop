# Task: Ragged softmax-pooling (uniform tiny len4)
...
Packed ragged data: `v` is `(1048576, 32)` float16 values, `s` is `(1048576,)` float16 scores, and
`offsets` is `(262145,)` int64 segment boundaries (262144 segments, none longer than 4).
...
Segment lengths follow `{'kind': 'uniform', 'len': 4}` (see `workload.py::lengths`). The total number of elements is
always exactly 1048576 and the tensor shapes never change; the pattern of lengths and all values
change with every call. The padded tensor the reference builds has 1x as many
